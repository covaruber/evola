...
Q <- pullQtlGeno(res0$pop, simParam = res0$simParam, trait=1); Q <- Q/2
bestid = names(sort(res0$fitness))[6]
best=which(rownames(Q)== bestid);best
betas <- res0$simParam$traits[[1]]@addEff[which(Q[best,] > 0)]
betas
...
```{r}
salesf <- function(Y,b,d,Q,D,a ,H, nCities){
...
Q <- pullQtlGeno(res$pop, simParam = res$simParam, trait=1); Q <- Q/2
bestid = names(sort(res$fitness, decreasing = FALSE))[1]; bestid # lowest first
best=which(rownames(Q)==bestid)[1]; best # best solution
Q[best,] # routes taken
