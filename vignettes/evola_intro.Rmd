...
best=bestSol(res0$pop)[1]; best 
# actual solution
res0$pop@Q[best,] 
# value and weight for the selected solution 
qa = res0$pop@Q[best,] %*% as.matrix(Gems[,c("Weight","Value")]); qa
```
...
sum(res$pop@Q[best,]) # total # of inds selected
```
...
```{r, fig.show='hold'}
pmonitor(res)
plot(DT$Yield, col=as.factor(res$pop@Q[best,]), 
     pch=(res$pop@Q[best,]*19)+1)
...
```{r}
# pmonitor(res)
# plot(DT$GY, col=as.factor(res$pop@Q[best,]), 
#        pch=(res$pop@Q[best,]*19)+1)
```
...
cex[names(which(res$pop@Q[best,]==1))]=2
...
# 
# best = bestSol(res$pop)[1]
# qa = (res$pop@Q %*% DT$GY)[best,]; qa 
# qDq = res$pop@Q[best,] %*% D %*% res$pop@Q[best,]; qDq 
# sum(res$pop@Q[best,]) # total # of inds selected
```
...
# crosses <- data.frame(cross=names(which( res$pop@Q[best,] == 1)))
...
error = ( stan(y) - apply( X*res0$pop@pheno,1,sum ) )^2
best=which(error == min(error))[1]
qa=res0$pop@Q[best,] %*% as.matrix(dt[,c("inter","alpha1")]); qa

plot( as.matrix(mtcars[,c("inter","hp")]) %*% t(qa)  , mtcars$mpg,
...
cor( as.matrix(mtcars[,c("inter","hp")]) %*% t(qa)  , mtcars$mpg) 
