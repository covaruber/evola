...
nqtls=100
dt <- data.frame(rnorm(nqtls),rnorm(nqtls),rnorm(nqtls),rnorm(nqtls) )
...
res0<-evolafit(formula=cbind(alpha1,alpha2,alpha3,alpha4)~qtl, dt= dt,
               # constraints: if greater than this ignore
               constraintsUB = c(Inf,Inf,Inf,Inf), 
               # constraints: if smaller than this ignore
               constraintsLB= c(-Inf,-Inf,-Inf,-Inf), 
               # weight the traits for the selection
               b = c(1,1,1,1), 
               # population parameters
               ...
               fitnessf=function(Y,b,d,Q,X,y){  
                 # version where each solution (b.i) needs to be evaluated against y - Xb.i
                 v <- 1:nrow(Y)
                 mse = apply( apply(y[v]%*%Jc(nrow(Y)),2,stan) - apply(as.matrix(X[v,])%*% t(Y),2,stan),
...
Xb_sols=apply(as.matrix(X)%*% t(as.matrix(res0$pop@gv)),2,stan)
y_sols=apply(y%*%Jc(nrow(res0$pop@gv)),2,stan)
e_sols=y_sols - Xb_sols
mse = apply(e_sols,2,function(x){sum(x^2)})
best=which(mse == min(mse))[1];best

# extract solutions if desired
Q <- pullQtlGeno(res0$pop, simParam = res0$simParam, trait=2); Q <- Q/2
# extract beta values
betas = res0$pop@gv[best,]; betas
