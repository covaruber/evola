...
dt <- data.frame(rnorm(400),rnorm(400),rnorm(400),rnorm(400) )
...
res0<-evolafit(formula=cbind(alpha1,alpha2,alpha3,alpha4)~qtl, dt= dt,
               ...
               fitnessf=function(Y,b,d,Q,X,y){  sum( (y) - apply(X*Y,1,sum) )^2 },
               # selection parameters
               propSelBetween = 0.65, propSelWithin =0.65, selectTop=FALSE,
               nGenerations = 45, y=y, X=X, verbose = FALSE
) 
...
best=which(error == min(error))[1];best
