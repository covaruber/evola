...
qa = res0@Q[best,] %*% as.matrix(Gems[,c("Weight","Value")]); qa
```
...
sum(res@Q[best,]) # total # of inds selected
```
...
```{r, fig.show='hold'}
pmonitor(res)
plot(DT$Yield, col=as.factor(res@Q[best,]), 
     pch=(res@Q[best,]*19)+1)
...
```{r}
# pmonitor(res)
# plot(DT$GY, col=as.factor(res@Q[best,]), 
#        pch=(res@Q[best,]*19)+1)
```
...
cex[names(which(res@Q[best,]==1))]=2
...
# 
# best = bestSol(res$pop)[1]
# qa = (res@Q %*% DT$GY)[best,]; qa 
# qDq = res@Q[best,] %*% D %*% res@Q[best,]; qDq 
# sum(res@Q[best,]) # total # of inds selected
```
...
# crosses <- data.frame(cross=names(which( res@Q[best,] == 1)))
...
qa=res0@Q[best,] %*% as.matrix(dt[,c("inter","alpha1")]); qa
