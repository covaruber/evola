...
sam <- sample(1:nrow(mtcars),1000,replace = TRUE)
y <- mtcars$mpg[sam]
one <- rep(1,length(y))
x <- mtcars$hp[sam]
x2 <- mtcars$hp[sam]^2
...
res0<-evolafit(formula=cbind(alpha1,alpha2)~qtl, dt= dt,
               # constraints: if greater than this ignore
               ...
               traitWeight = c(1,1), 
               # population parameters
               nCrosses = 100, nProgeny = 10, recombGens = 1, 
               # coancestry parameters
               A=NULL, lambda=0, nQTLperInd = 1, 
               fitnessf=function(Y,b,d,x,y){ ( y - apply( X*Y,1,sum ) )^2 },
               # selection parameters
               propSelBetween = 0.5, propSelWithin =0.5, selectTop=FALSE,
               nGenerations = 20, dateWarning=TRUE, y=y, x=x
) 
...
plot( as.matrix(mtcars[,c("inter","hp")]) %*% t(xa)  , mtcars$mpg) # GA
plot( (mtcars$hp * mod$coefficients[2] ) + mod$coefficients[1] , mtcars$mpg) # LM

cor( as.matrix(mtcars[,c("inter","hp")]) %*% t(xa)  , mtcars$mpg) # GA
